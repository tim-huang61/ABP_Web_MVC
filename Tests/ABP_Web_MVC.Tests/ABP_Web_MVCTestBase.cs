...
namespace ABP_Web_MVC.Tests
{
    public abstract class ABP_Web_MVCTestBase : AbpIntegratedTestBase<ABP_Web_MVCTestModule>
    {
        ...
        protected void UsingDbContext(Action<ABP_Web_MVCDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABP_Web_MVCDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABP_Web_MVCDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABP_Web_MVCDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABP_Web_MVCDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABP_Web_MVCDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABP_Web_MVCDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABP_Web_MVCDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABP_Web_MVCDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABP_Web_MVCDbContext, Task<T>> func)
        {
            T result;
